...
代码写的我有点看不懂，因为有的processor可以合并处理，比如这里的EarlyEntryPointProcessor可以和AggregatedDepsProcessor放在一起。唯一的解释就是这里的processor后面还会进行扩展，所以单独拎出来了。当然我们没必要纠结，下面就是最核心模块，在`dagger.hilt.processor.internal.root`包下。
