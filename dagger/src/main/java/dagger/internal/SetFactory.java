...
public final class SetFactory<T> implements Factory<Set<T>> {
    ...
    public static final class Builder<T> {
        ...
        private Builder(int individualProviderSize, int collectionProviderSize) {
            individualProviders = DaggerCollections.presizedList(individualProviderSize);
            collectionProviders = DaggerCollections.presizedList(collectionProviderSize);
        }
    ...
    public Set<T> get() {
        ...
        //分析表明，这种方法在某些应用程序中是一个 CPU 消耗热点，因此这些循环被更改为使用 c 样式。
        // 与增强的 for-each 循环相比，C 风格的 for ArrayLists 更快，至少通过 Java 8。
